-- ============================================================================
-- 数据库: db_systar (H2)
-- 说明: 与 sql/mysql/data/01-init.sql 对应，使用 H2 兼容语法（MERGE INTO 代替 ON DUPLICATE KEY）
-- ============================================================================
...
-- ============================================================================
-- 11. 演示用资产树
-- 与 sql/mysql/data/01-init.sql 内容一致，使用 H2 MERGE INTO 语法
-- ============================================================================

-- 11.1 监控服务
MERGE INTO t_service (id, name, caption, mode, driver_class, max_connections, type_name) KEY (id)
    VALUES (100, 'modbus_tcp_svc',  'Modbus TCP服务',  0, 'com.systar.monitor.drivers.modbus.ModbusService',    10, 'ModbusTcpMaster'),
...
-- 11.2 设备
MERGE INTO t_device (id, name, caption, catalog, vendor, purchase_date, warranty_date, health_index) KEY (id)
    VALUES (1001, 'th_sensor_001',   '温湿度传感器-001',   201, 'Systar',   '2025-01-15 00:00:00', '2028-01-15', 0.95),
...
-- 11.5 统一资产视图
MERGE INTO t_asset (id, name, caption, kind, type_id, parent_id, state, enabled, sort, device_id, service_id, probe_id, control_id) KEY (id)
    VALUES (10, 'modbus_tcp_svc',   'Modbus TCP服务',  2, 103, 0,    0, 1, 1, NULL, 100,  NULL,   NULL),
